...
bool bt, gtmp, gmea, mov, dbc, lrcb, ps, fs, skip, ppmy, txtoff, playrd100t, playrd100t2, *buf, keysig2, stop, damppad[16], softpad[16], sti3, stop2, g3446 = true, atmod, wjaa, updta, vol, strclrc, ply = true, f, fnm, pb, quit, htit, mmp, forcenomidi, sdfsdf, exbl;
...
inline void getini() {
	...
	if (access(stf, 0) != 0) {
		...
		turfreq = 440.0f;
		fsize = 20; fspace = 100;
		strcpy(jk678[13], "Arial");
		
		ini = fopen(stf, "w");
		printf("5");
		for (int i = 0; i < 12; i++) fprintf(ini, "_\n");
		...
		fprintf(ini, "%d %d %d %d %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x_\n", windowx, windowy, windh, windv, mmp, t[0].b, t[1].b, t[2].b, t[3].b, t[4].b, t[5].b, t[6].b, 350, 100, textmode, h54ui, jhj56, ghkj6, j65j8, trand, mirror, reverse, knamei, kfreqi, hortxti, t[7].b, t[8].b, t[9].b, t[10].b, fsize, fspace, t[11].b, t[12].b, t[13].b, t[14].b, txtmd2, t[15].b);
		fprintf(ini, "1\n");
		for (int i = 0; i < 16; i++) {
			t[0].a = (float)(getr(sbcolor[i])) / 256;
			t[1].a = (float)(getg(sbcolor[i])) / 256;
			t[2].a = (float)(getb(sbcolor[i])) / 256;
			t[3].a = 0.99609375;
			fprintf(ini, "%x %x %x %x\n", t[0].b, t[1].b, t[2].b, t[3].b);
		}
		fprintf(ini, "Arial_\n");
	...
	fscanf(ini, " %[^\n]", jk678[13]);
	//printf("%d ", ftell(ini));
	jk678[13][strlen(jk678[13]) - 1] = 0;
	FILE *tmp;
	int a = 0;
	for (int i = 0; i < 12; i++) {
		if ((tmp = fopen(jk678[i], "r")) == NULL) {
			a = 1;
			jk678[i][0] = 0;
		}
		else fclose(tmp);
	...
	fsize = 0; fspace = 0;
	//("%d %d\n", fsize, fspace);
	sscanf(jk678[12], " %d %d %d %d %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x", &windowx, &windowy, &windh, &windv, &mmp, &t[0].b, &t[1].b, &t[2].b, &t[3].b, &t[4].b, &t[5].b, &t[6].b, &asfaq, &brebe, &textmode, &h54ui, &jhj56, &ghkj6, &j65j8, &trand, &mirror, &reverse, &knamei, &kfreqi, &hortxti, &t[7].b, &t[8].b, &t[9].b, &t[10].b, &fsize, &fspace, &t[11].b, &t[12].b, &t[13].b, &t[14].b, &txtmd2, &t[15].b);
	//printf("!%s!\n", jk678[12]);
	if (a)putini();
	//FILE *tmp;
...
inline int LoadGLTextures(void) {
	...
	if ((texRec = LoadBMPFile(imgname))) {
		...
		glTexImage2D(GL_TEXTURE_2D, 0, 3, texRec->sizeX, texRec->sizeY, 0,
			...
			GL_RGB, GL_UNSIGNED_BYTE, texRec->data); //<9>
...
inline void noteoff(int wyr, int midichennel) {
	//if (pkey4[wyr][midichennel] > 0) pkey4[wyr][midichennel]--;
	//if (pnote == -1) system("pause");
...
inline void noteon(int wyr, int vel, int chennel, int midichennel) {
	...
	//if (midichennel == 9) MessageBox(0,"A", "A", MB_OK);
	if (mirror)wyr = 0x7F - wyr;
	note++;
...
inline void getmidieventacc() {
	...
	for (int j = 0; j < chn; j++) {
		//if(j<16) printf("!! %d %lf %d\n", chorus[j], dchorus[j],j);
		unsigned int tmp2;
		...
		C2:
			...
			if (tmp <= 0x7F) { //("!"); 
				...
				//printf("%s ", parsetmp[((unsigned long)gmetmp2 << 7) + 60][0][0].achSampleName);
			}
			...
			while (1) {
				...
				if (tmp == 0) {
					tmp = sjgf(ch, j);
				C1:
					//printf("%x_%d ", (int)(fileptr[j] - 8000 + chbuffercnt[j]), j);
					//Sleep(10);
					gmetmp2 = tmp & 15;
					if (tmp <= 0x7F) { //("!"); 
	...
	for (int j = 0; j < chn; j++) {
		...
		if (buf[j] == false) {
			while (1) {
				//printf("%x_%d ", (int)(fileptr[j]-8000+chbuffercnt[j]),j);
				tmp = sjgf(ch, j);
				//printf("c%x_%d_%x ", (int)(fileptr[j] - 8000 + chbuffercnt[j])-1, j, tmp);
				if (tmp == 0) {
					tmp = sjgf(ch, j);
					//printf("!%x_%d ", (int)(fileptr[j] - 8000 + chbuffercnt[j]), j);
				C3:
...
inline int clock2_save() {
	_clock2 += spd / 100.*(double)(clock() - clock_start);
	clock_start = clock();
	return 0;
}
inline int clock2() {
...
DWORD WINAPI playmidi(LPVOID unused)
{
	...
	while (1)
	{
		...
		while (clock2() - playmidin0 < playmidii) { Sleep(1); }
		while (len--)
		{
...
void loadimg() {
	...
	//
	FILE *u;
	if (strcmp(jk678[10], "\0") != 0 && (u = fopen(jk678[10], "r")) != 0) {
		fclose(u);
		tpi = 1;
		//("%s\n", jk678[10]);
		strcpy(imgname, jk678[10]);
		//("%s\n",imgname);
...
void mtinit2() {
	...
	for (int i = 0; i < playoffset / 2; i++) {
		fscanf(wj6, "%s", tsb1); fscanf(wj6, "%s", tsb1); fscanf(wj6, "%s", tsb1); fscanf(wj6, "%s", tsb1); fscanf(wj6, "%s", tsb1);
	}
	printf("V");
...
int exit2(int iex) {
	if (!midiout) {
		...
		fprintf(t, "ping 127.0.0.1 -n 1\ndel \"%s\"\ndel \"%s\"\ndel \"%s\"\ndel a.bat", tsb0, tsb2, tsb3);
		fclose(t);
		ShellExecute(0, "open", "a.bat", 0, 0, 0);
		//MessageBox(tsb0, "A", MB_OK);
		//MessageBox("C", "C", MB_OK);
	}
	else {

		FILE *t = fopen(tsb3, "rb");
		if (t == NULL) goto IUI2O;
		_fseeki64(t, -11, SEEK_END);
		if (fgetc(t) != 'e') {
			fclose(t);
			t = fopen("a.bat", "w");
			fprintf(t, "ping 127.0.0.1 -n 1\ndel \"%s\"\ndel \"%s\"\ndel \"%s\"\ndel a.bat", tsb, tsb2, tsb3);
			fclose(t);
			ShellExecute(0, "open", "a.bat", 0, 0, 0);
...
#pragma omp parallel for
	...
	//printf("!");
	/**
	isplay=0;mciSendString("pause wave", NULL, 0, NULL);
	...
	if (argv[chn - 4] == '.'&&argv[chn - 3] == 'h'&&argv[chn - 2] == 'm'&&argv[chn - 1] == 'p')argv[chn - 8] = 0;
	{
		...
		if (y == NULL) {
			if (kj768 == true)
				MessageBox("에러-파일을 열 수 없음", "하이온의 미디 플레이어 v5.0", MB_OK | MB_ICONSTOP);
			else
	...
	strcpy(jk678[0], argv);
	{
		...
		FILE* w;
		while (u != 10) {
			if ((w = fopen(jk678[u], "r")) != NULL) {
				for (int uv = 0; uv <= v; uv++) if (strcmp(tmplst[uv], jk678[u]) == 0) goto UTV;
				strcpy(tmplst[v], jk678[u]); v++;
			}
		UTV:
			u++;
		}
		for (u = 0; u < 10; u++) strcpy(jk678[u], tmplst[u]);
	}
	//printf("!");
	putini(); ////printf("!");
			  //printf("?");
	...
	//printf("!");
	//printf("%s\n", tsb7);
	tsb7[strlen(tsb7) - 3] = 0; //asfaq = 0; brebe = 0;
	strcat(tsb7, "lrc");
	...
	//printf("?");
	//printf("!");
	//printf("%s %s %s %s %s\n",t, tsb, tsb2, tsb3, tsb4);
	int fasfafafqw = 0;
	if ((i = fopen(tsb3, "r")) != NULL) {
		char temp;
...
M:
	...
	if (mode == 12 || mode == 13) {
		...
		if (!midiout) {
			...
			fclose(wj6);
			fclose(wj7);
			sprintf(a, "del \"%s\"", tsb3);
			system(a);
			sprintf(a, "del \"%s\"", tsb0);
			system(a);
			printf("!%s", a);
...
#pragma omp parallel for
	...
	if (chn == 0) {
		if (kj768 == true) {
			MessageBox("에러-깨진 미디 파일임", "하이온의 미디 플레이어 v5.0", MB_OK | MB_ICONSTOP); Sleep(1000); if (!midiout) {
				//	MessageBox("B", "B", MB_OK);
				//cmidi.Pause();
				//Sleep(100);
				TerminateThread(thread1, 0);
			...
			MessageBox("error-broken midi file", "hyon's midi player v5.0", MB_OK | MB_ICONSTOP); Sleep(1000); if (!midiout) {
				//	MessageBox("B", "B", MB_OK);
				//cmidi.Pause();
				TerminateThread(thread1, 0);
				CloseHandle(thread1);
	...
	printf("))");
	//("%d %d %d\n", le2[0], le2[1], le2[2]);
	//lplayrate = (unsigned long long)playrate;

	//if (sfn == 1) mode = 72;

	SetTimer(1, 20, NULL);
	//("%d", (60 * playrate) / (deltatime * tempo));
	//	readpc(pc3, chn);
...
inline void CCubeView::arecloop() {
	...
	fwrite("eof", 1, 3, wj5);
	fwrite(&tsb3l, 8, 1, wj5);

	fclose(wj4); fclose(wj5);
	if (reverse == true) {
		revfile(tsb3); revfile2(tsb);
	}
...
int CCubeView::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	if (CView::OnCreate(lpCreateStruct) == -1)
		return -1;
	for (int i = 0; i < __argc; i++)printf("! %s!\n", __argv[i]);
	...
	//("*");
	if (kj768 == true)
		GetParent()->SetWindowTextA("하이온의 미디 플레이어 v5.0");
	else
				...
				if (owptr6 == playrd100) {
					...
					if (wptr - owptr4 == playrd10) {
						if (GetTickCount64() - owptr2 > 2000) {
							...
							if (kj768 == true)
								sprintf(t, "하이온의 미디 플레이어 v5.0 -불러오는 중:%.1lfx", (clocks2 - clocks));
							else
						...
						if (fnm);
							...
							GetModuleFileNameA(NULL, st, 1000);
							sprintf(st2, "\"%s\"", st);
							if (ply == false) sprintf(st3, "\"%s\"_f__p_", midname);
							else sprintf(st3, "\"%s\"_f_", midname);
							for (int i = 2; i < __argc; i++) {
								strcat(st3, " \"");
								strcat(st3, __argv[i]);
								strcat(st3, "\"");
...
#pragma omp parallel for
					for (int j = 0; j < 16; j++) {
						...
						//	//printf("%x %x %x \n", ktemp[j << 1], ktemp[(j << 1) + 1]);
					}

					for (int i = 0; i < 16; i++) {
						for (int j = 0; j < 64; j++) {
							ktemp[(i << 6) + j + 32] = (key3[(j << 1) + 1][i] << 4) | key3[j << 1][i];
							//	//printf("%x %x %x\n",(j << 2), key3[(j << 1) + 1][i], key3[j << 1][i]);
						}
					}
...
#pragma omp parallel for
				...
				//fprintf(wj, "%c%c%c%c", wjct2.c[0], wjct2.c[1], wjct2.c[2], wjct2.c[3]);

				//("%x %x %x %x\n", wjct2.c[0],wjct2.c[1],wjct2.c[2],wjct2.c[3]);
			...
			if (f == true && onelp == 0) {
				...
				if (ply == true) {
					...
					//else cmidi.SetPos(50 * playoffset);
			...
			else if (onelp == 1) onelp = 0;
			//printf("B");
			if (owptr6 < 24 * 2 && !midiout) {
				//printf("!");
				playoffset = 0;
				if (isplay) {
					// cmidi.MoveStartPos(), cmidi.Pause();
					isplay = 0;
				}

				//waveOutSetPosition(0);
			}
			if (owptr6 == 24 * 2 && !midiout) {
				playoffset = 0;
				gtc = clock();
				...
				ii += clock() - gtc;
				oplaymidii = 0;
				//PlaySoundA(tsb3, NULL, SND_FILENAME | SND_ASYNC);
			...
			if (owptr6 > 24 * 2 && !midiout && oplaymidii != playmidii) {
				printf("\n%lld %lld %lld\n", playmidii, playoffset * 50, playoffset * 50 - (long long)playmidii);
			...
			//printf("%d ", 50 * playoffset);
			//printf("C");
			if (owptr6 < 80) owptr6++;
			/*
			_ftelli64(wj7) < offs.i  ? fgetc(wj7) : 0
			*/
			...
			//printf("!%d %d %d\n", cmidi.GetPos(), (int)playoffset * 50,(int)GetTickCount());
			...
			for (int z = 0; z <5; z++) {
				...
				//("%d %d \n", pl, ph);
				if (_ftelli64(wj7) < offs.i)
					fread(tsb8, 1, 1024, wj7);
				else
					memset(tsb8, 0, 1024);
				//printf("%x %x!\n ", (long)_ftelli64(wj7), (long)offs.i);
				q = -1;
				...
				for (int x = 0; x < 16; x++) {
					...
					for (int yi2 = 0; yi2 < 268; yi2++) {
						if ((mdtpnt + z) < 500)
							mdt[mdtpnt + z][yi2][x] =
							((yi2 >= 60 + pc2[x]) && (yi2 < 188 + pc2[x])) ?
							(((bt = !bt) == true) ? (tsb8[++q] & 0x0F) : (tsb8[q] >> 4))
							: 0;
						else
			...
			if (lt == 0) {
				if (tsb7[0] != 0) {
					if (kj768 == true)
						sprintf(t, "하이온의 미디 플레이어 v5.0 -%s/%dfps/%s/%s", midname, fps, midstr, tsb7);
					else
					...
					//////printf("%x", tsb7[0]);
					if (kj768 == true)
						sprintf(t, "하이온의 미디 플레이어 v5.0 -%s/%dfps/%s", midname, fps, midstr);
					else
			...
			if (ps == true && owptr6 > 25 * 2) {
				...
				if (lt == 0) { fscanf(wj6, "%s", tsb1); fscanf(wj6, "%s", tsb4); fscanf(wj6, "%s", tsb6); fscanf(wj6, "%s", midstr); fscanf(wj6, "%s", tsb7); }
				for (int i = 0; i < strlen(tsb7); i++) {
					if (tsb7[i] == 0x10) tsb7[i] = 0x20;
				}
				goto EX;
			}
			if (clock() - ocl>4000) {
			...
			if (owptr6 > 25 * 2 || fs == true) {
				...
				if (hortxti == 0) {
					...
					if (textmode % 2 == 0) { printchr(asfaq, brebe, windx, windy, tsb1, 0);  txtys += txtystmp; }
					if (lt == 0)	fscanf(wj6, "%s", tsb4);
					if (textmode != -1) {
						printchr(asfaq, brebe, windx, windy, tsb4, txtys);  txtys += txtystmp;
					}
					if (lt == 0)	fscanf(wj6, "%s", tsb6);
					if (textmode == 0 || textmode > 2) { printchr(asfaq, brebe, windx, windy, tsb6, txtys);  txtys += txtystmp; }
					if (lt == 0)	fscanf(wj6, "%s", midstr);
					//if (textmode == 0) { printchr(asfaq, brebe, windx, windy, , txtys);  txtys += fsize*fspace/143; }
					if (lt == 0)	fscanf(wj6, "%s", tsb7);
					if (textmode == 0 || textmode > 2) { printchr(asfaq, brebe, windx, windy, tsb7, txtys);  txtys += txtystmp; }
				}
				else if (hortxti == 1) {
					txtystmp = fsize*fspace / 1000;
					if (textmode % 2 == 0) { printchr(asfaq + windx / 15 * txtys, brebe, windx, windy, tsb1, 0); txtys += txtystmp; }
					if (lt == 0)	fscanf(wj6, "%s", tsb4);
					if (textmode != -1) { printchr(asfaq + windx / 15 * txtys, brebe, windx, windy, tsb4, 0); txtys += txtystmp; }
					if (lt == 0)	fscanf(wj6, "%s", tsb6);
					if (textmode == 0 || textmode > 2) { printchr(asfaq + windx / 15 * txtys, brebe, windx, windy, tsb6, 0); txtys += txtystmp; }
					if (lt == 0)	fscanf(wj6, "%s", midstr);
					//if (textmode == 0) { printchr(asfaq + windx / 15 * txtys, brebe, windx, windy, , 0); txtys += 3; }
					if (lt == 0)	fscanf(wj6, "%s", tsb7);
					if (textmode == 0 || textmode > 2) { printchr(asfaq + windx / 15 * txtys, brebe, windx, windy, tsb7, 0); txtys += txtystmp; }
				}
				else {
					htit = true;
					memset(tsb9, ' ', 400);
					if (textmode % 2 == 0) { strcpy(tsb9 + txtys, tsb1); tsb9[strlen(tsb9)] = ' '; txtys += 30; }
					if (lt == 0)	fscanf(wj6, "%s", tsb4);
					if (textmode != -1) { strcpy(tsb9 + txtys, tsb4); tsb9[strlen(tsb9)] = ' '; txtys += 20; }
					if (lt == 0)	fscanf(wj6, "%s", tsb6);
					if (textmode == 0 || textmode > 2) { strcpy(tsb9 + txtys, tsb6); tsb9[strlen(tsb9)] = ' '; txtys += 20; }
					if (lt == 0)	fscanf(wj6, "%s", midstr);
					//if (textmode == 0) { printchr(asfaq + windx / 15 * txtys, brebe, windx, windy, , 0); txtys += 3; }
					if (lt == 0)	fscanf(wj6, "%s", tsb7);
					if (textmode == 0 || textmode > 2) { strcpy(tsb9 + txtys, tsb7); tsb9[strlen(tsb9)] = ' '; txtys += 20; }
					print2st(tsb9);
				}
				for (int i = 0; i < strlen(tsb7); i++) {
					if (tsb7[i] == 0x10) tsb7[i] = 0x20;
				...
				if (hortxti == 0) {
					...
					if (textmode == 0 || textmode > 2) { printchr(asfaq, brebe, windx, windy, tsb6, txtys);  txtys += txtystmp; }
					//if (lt == 0)	fscanf(wj6, "%s", midstr);
					//if (textmode == 0) { printchr(asfaq, brebe, windx, windy, tsb1, txtys);  txtys += fsize*fspace/143; }
					//	if (lt == 0)	fscanf(wj6, "%s", tsb7);
					if (textmode == 0 || textmode > 2) { printchr(asfaq, brebe, windx, windy, tsb7, txtys);  txtys += txtystmp; }
				}
				else if (hortxti == 1) {
					...
					if (textmode == 0 || textmode > 2) { printchr(asfaq + windx / 15 * txtys, brebe, windx, windy, tsb6, 0); txtys += txtystmp; }
					//if (lt == 0)	fscanf(wj6, "%s", midstr);
					//if (textmode == 0) { printchr(asfaq + windx / 15 * txtys, brebe, windx, windy, tsb1, 0); txtys += 3; }
					//if (lt == 0)	fscanf(wj6, "%s", tsb7);
					if (textmode == 0 || textmode > 2) { printchr(asfaq + windx / 15 * txtys, brebe, windx, windy, tsb7, 0); txtys += txtystmp; }
				}
				else {
		...
		EX:
			...
			if (hortxti == 0) {
				txtystmp = fsize*fspace / 143;
				if (textmode % 2 == 0) { printchr(asfaq, brebe, windx, windy, otsb2, 0);  txtys += txtystmp; }
				if (textmode != -1) { printchr(asfaq, brebe, windx, windy, tsb4, txtys);  txtys += txtystmp; }
				if (textmode == 0 || textmode > 2) { printchr(asfaq, brebe, windx, windy, tsb6, txtys);  txtys += txtystmp; }
				//if (textmode == 0) { printchr(asfaq, brebe, windx, windy, tsb1, txtys);  txtys += fsize*fspace/143; }
				if (textmode == 0 || textmode > 2) { printchr(asfaq, brebe, windx, windy, tsb7, txtys);  txtys += txtystmp; }
			}
				...
				txtystmp = fsize*fspace / 1000;
				if (textmode % 2 == 0) { printchr(asfaq + windx / 15 * txtys, brebe, windx, windy, otsb2, 0); txtys += txtystmp; }
				//	if (lt == 0)	fscanf(wj6, "%s", tsb4);
				if (textmode != -1) { printchr(asfaq + windx / 15 * txtys, brebe, windx, windy, tsb4, 0); txtys += txtystmp; }
				//	if (lt == 0)	fscanf(wj6, "%s", tsb6);
				if (textmode == 0 || textmode > 2) { printchr(asfaq + windx / 15 * txtys, brebe, windx, windy, tsb6, 0); txtys += txtystmp; }
				...
				htit = true;
				memset(tsb9, ' ', 400);
				if (textmode % 2 == 0) { strcpy(tsb9 + txtys, otsb2); tsb9[strlen(tsb9)] = ' '; txtys += 30; }
				//		if (lt == 0)	fscanf(wj6, "%s", tsb4);
				if (textmode != -1) { strcpy(tsb9 + txtys, tsb4); tsb9[strlen(tsb9)] = ' '; txtys += 20; }
				//		if (lt == 0)	fscanf(wj6, "%s", tsb6);
				if (textmode == 0 || textmode > 2) { strcpy(tsb9 + txtys, tsb6); tsb9[strlen(tsb9)] = ' '; txtys += 20; }
...
{
	printf("*%d*", nChar);//MessageBox("A", "A", MB_OK);
...
void CCubeView::OnFileOpen32773()
{
	...
	if (IDOK == dlg.DoModal()) {
		...
		printf("~%s~", strPathName.GetBuffer());
...
void CCubeView::OnViewSetbackgroundimage()
{
	...
	if (IDOK == dlg.DoModal()) {
		...
		strcpy(jk678[10], strPathName.GetBuffer());
		putini();
		loadimg();
	}
...
void CCubeView::OnPlayPitchdown()
{
	...
	pitch[9] = 100.;
	if (kj768 == true)
		sprintf(t, "하이온의 미디 플레이어 v5.0 - 음높이 -1");
	else
...
void CCubeView::OnPlaySkipback()
{
	...
	if (playoffset >= 200) {
		if (skip == false) {
			...
			//if (playoffset < 0) playoffset = 0;
			if (kj768 == true)
				sprintf(t, "하이온의 미디 플레이어 v5.0 -오프셋 %.2lf초", (float)playoffset / 20, '%');
			else
...
void CCubeView::OnPlaySpeeddown()
{
	...
	if (kj768 == true)
		sprintf(t, "하이온의 미디 플레이어 v5.0 -재생 속도 %d%c", (int)(spd), '%');
	else
...
void CCubeView::OnPlayVolumedown()
{
	...
	if (kj768 == true)
		sprintf(t, "하이온의 미디 플레이어 v5.0 - 소리 크기 %d%c", (int)(playvolume * 100), '%');
	else
...
void CCubeView::OnPlayReset()
{
	...
	bpshf = 0;
	if (kj768 == true)
		sprintf(t, "하이온의 미디 플레이어 v5.0 -속도 100%c,소리 크기 100%c", '%', '%');
	else
