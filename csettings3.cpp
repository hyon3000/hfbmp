...
void csettings3::OnTimer(UINT_PTR nIDEvent)
{
	...
	if (GetFocus()->GetDlgCtrlID() != IDC_d1_pv2) {
		if (pchn >= 0) {
			sprintf(p, "%+.5lf", 12.*log2(pitch[pchn] / 100.));
		}
		else {
	...
	if (GetFocus()->GetDlgCtrlID() != IDC_d1_sc) {
		csc = dbl2charp(spd);
	}
	if (GetFocus()->GetDlgCtrlID() != IDC_COMBO2 && GetFocus()->GetDlgCtrlID() != 1001 ) {
		if (ichn >= 0) {
	...
	if (GetFocus()->GetDlgCtrlID() != IDC_d1_bv) {
		if (bchn >= 0) {
	...
	if (GetFocus()->GetDlgCtrlID() != IDC_vol_master) {
		cvv_mast = dbl2charp(playvolume*100.);
	}
	if (GetFocus()->GetDlgCtrlID() != IDC_d1_sc2&&GetFocus()->GetDlgCtrlID() != IDC_SLIDER1) {
		sprintf(p, "%.2lf / %.2lf", (double)playoffset / 20., (double)playlength / 100.);
		cposit = p;
	}
...
void csettings3::OnChangeD1Vvbass()
{
	...
	for (int i = 0; i < 128; i++)printf("%lf ", mdvol3[i][0]);
	// TODO:  여기에 컨트롤 알림 처리기 코드를 추가합니다.
	UpdateData(0);
}
